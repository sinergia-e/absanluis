...
class OrgChartDepartment(models.Model):
	...
	def _get_image(self, emp, style):
		...
		image_path = "/org_chart_employee_pro/static/src/img/default_image.png"
		return '<img class="avatar %s" src=%s />' % (style, image_path)
	...
	def get_emp_form_id(self):
		return {'form_id': self.env.ref('org_chart_employee_pro.chart_employee_form').id}
...
class SlifeEmployee(models.TransientModel):
	...
	def _default_image(self):
		image_path = get_module_resource('org_chart_employee_pro', 'static/src/img', 'default_image.png')
